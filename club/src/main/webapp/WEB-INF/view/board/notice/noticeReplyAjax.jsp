...
<table id="reply">
			...
			%>
				<tr id="re_info"> 
					...
					<td id="repl_del"><input type="button" value="삭제" onclick="location.href='/board/replyDelete.do?reply_num=<%=list.get(i).getReply_num() %>&post_id=<%=list.get(i).getPost_id()%>'"></td>
				</tr>
				...
				<input type="hidden" name="g_id" value="<%=list.get(i).getG_id()%>">
				<input type="hidden" name="g_lev" value="<%=list.get(i).getG_lev()%>">
				<input type="hidden" name="g_seq" value="<%=list.get(i).getG_seq()%>">
				<tr class="re_tr">
					<td id="dndn">└</td>
					<td>
					...
					<td> 
						<input type="button" class="repl_btn" value="등록" onclick="replyAjax2();"> 
					</td>
				</tr>
				</form>
			<%
