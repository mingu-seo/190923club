<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
  <div class="visualLeftInner">
	        		<div class="leftMenu">
		        		<div class="board-title">갤러리</div> <!-- 위치수정가능하게 -->
			        	<div>
			        		<ul>
			        			<li><a href=""><span class="ctg-s">┗</span>생일</a></li> <!-- 내용수정가능하게/추가삭제 가능하게 -->	
			        			<li><a href=""><span class="ctg-s">┗</span>대회</a></li> <!-- 내용수정가능하게/추가삭제 가능하게 -->	
			        			<li><a href=""><span class="ctg-s">┗</span>회식</a></li> <!-- 내용수정가능하게/추가삭제 가능하게 -->	
			        		</ul>
						</div>
	        		</div>
	        		
	        		
	        		
        			<div class="leftMenu">
		        		<div class="board-title">게시판</div> <!-- 위치수정가능하게 -->
		        		<div>
		        			<ul>
		        				<li><a href=""><span class="ctg-s">┗</span>공감해주셈</a></li> <!-- 내용수정가능하게/추가삭제 가능하게 -->
		        				<li><a href=""><span class="ctg-s">┗</span>웃긴글</a></li> <!-- 내용수정가능하게/추가삭제 가능하게 -->	
		        				<li><a href=""><span class="ctg-s">┗</span>어쩌구</a></li> <!-- 내용수정가능하게/추가삭제 가능하게 -->	
		        			</ul>
	        			</div>
	        		</div>
	        			
	        		
	        		
        			<div class="leftMenu">
						<div class="board-title">공지사항</div>
						<div>
							<ul>
							</ul>
						</div>
						
	        		</div>
        		
	        		<div class="clubInfo">
	        		가입신청
	        		</div>
	        		
	        		<div class="clubInfo">
	        		동아리 설정
	        		<a href=""><img src="/club/img/set.png"></a>
	        		</div>
	        		
	</div>